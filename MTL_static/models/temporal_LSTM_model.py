...
class Multitask_EmotionNet(InceptionV3MTModel):
    ...
    def configure_architecture(self):
        ...
        self.transformation_LSTMs = []

        for i_au in range(len(self.au_names_list) + len(PRESET_VARS.Hidden_AUs)):
            lstm_layer = nn.LSTM(self.AU_metric_dim, self.AU_metric_dim, num_layers = 1, batch_first = True)
            self.transformation_LSTMs.append(lstm_layer)
        self.transformation_LSTMs = nn.ModuleList(self.transformation_LSTMs)
    ...
    def forward(self, x):
        ...
        feature_maps = self.backbone_CNN(x.view((bs*seq_len, *x.size()[2:])))
        ...
        AU_metrics = torch.stack(AU_metrics, dim=1)  # size: bs*seq_len, N_aus, dim
        ...
        outputs['AU'] = self.emotion_classifiers[i_classifier](AU_metrics_with_labels).view((bs, seq_len, -1))
        metrics['AU'] = AU_metrics_with_labels.view((bs, seq_len, -1))

        # rotate the AU embeddings by learned rotation matrices
        EXPR_VA_metrics = []
        for i_au in range(len(self.au_names_list) + len(PRESET_VARS.Hidden_AUs)):
            au_metric = AU_metrics[:, i_au, ...] # (bs*seq_len, dim)
            projected, _ = self.transformation_LSTMs[i_au](au_metric.view((bs, seq_len, -1)))
            EXPR_VA_metrics.append(projected)
        EXPR_VA_metrics = torch.stack(EXPR_VA_metrics, dim=-2) # bs, seq_len, N_aus, dim 
        if not self.avg_features:
            bs, seq_len, length = EXPR_VA_metrics.size(0), EXPR_VA_metrics.size(1), EXPR_VA_metrics.size(2)
            # EXPR classifier
            i_classifier = self.tasks.index('EXPR')
            outputs['EXPR'] = self.emotion_classifiers[i_classifier](EXPR_VA_metrics.view((bs*seq_len*length, -1))).view((bs, seq_len, length, -1))
            metrics['EXPR'] = EXPR_VA_metrics

            # VA classifier
            i_classifier = self.tasks.index('VA')
            outputs['VA'] = self.emotion_classifiers[i_classifier](EXPR_VA_metrics.view((bs*seq_len*length, -1))).view((bs, seq_len, length, -1))
            metrics['VA'] = EXPR_VA_metrics
        else:
            EXPR_VA_metrics = EXPR_VA_metrics.mean(-2)
            bs, seq_len = EXPR_VA_metrics.size(0), EXPR_VA_metrics.size(1)
            # EXPR classifier
            i_classifier = self.tasks.index('EXPR')
            outputs['EXPR'] = self.emotion_classifiers[i_classifier](EXPR_VA_metrics.view((bs*seq_len, -1))).view((bs, seq_len, -1))
            metrics['EXPR'] = EXPR_VA_metrics

            # VA classifier
            i_classifier = self.tasks.index('VA')
            outputs['VA'] = self.emotion_classifiers[i_classifier](EXPR_VA_metrics.view((bs*seq_len, -1))).view((bs, seq_len, -1))
            metrics['VA'] = EXPR_VA_metrics
    ...
    def training_step(self, batch, batch_idx):
        ...
        for task in self.tasks:
            ...
            bs, seq_len = preds_task.size(0),preds_task.size(1)
            if task == 'AU':
                loss = self.training_task(preds_task.view((bs*seq_len, -1)), labels_task.view((bs*seq_len, -1)), metrics_task, task)
            else:
                reshape = (bs*seq_len, -1) if task == 'VA' else (bs*seq_len)
                if not self.avg_features:
                    loss = self.training_task(preds_task.mean(-2).view((bs*seq_len, -1)), labels_task.view(reshape), metrics_task, task)
                else:
                    loss = self.training_task(preds_task.view((bs*seq_len, -1)), labels_task.view(reshape), metrics_task, task)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx):
        ...
        if not self.avg_features:
            return torch.cat([preds['AU'], preds['EXPR'].mean(-2), preds['VA'].mean(-2)], dim=-1), y
        else:
            return torch.cat([preds['AU'], preds['EXPR'], preds['VA']], dim=-1), y
    def validation_single_task(self, dataloader_idx, preds, labels):
        # import pdb; pdb.set_trace()
        bs, seq_len = labels.size(0), labels.size(1)
        preds = dict([(k, preds[k].view((bs*seq_len, -1))) for k in preds])
        labels = labels.view((bs*seq_len, -1)) if len(labels.size())!=2 else labels.view((bs*seq_len,))
