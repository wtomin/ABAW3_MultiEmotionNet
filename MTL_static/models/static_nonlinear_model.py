...
class Multitask_EmotionNet(InceptionV3MTModel):
    ...
    def configure_architecture(self):
        ...
        for i_au in range(len(self.au_names_list) + len(PRESET_VARS.Hidden_AUs)):
            layer = nn.Sequential(
                nn.Linear(self.AU_metric_dim, self.AU_metric_dim, bias=True),
                nn.ReLU(),
                nn.Linear(self.AU_metric_dim, self.AU_metric_dim, bias=True))
            self.transformation_layers.append(layer)
        self.transformation_layers = nn.ModuleList(self.transformation_layers)
    ...
    def forward(self, x):
        ...
        for i_au in range(len(self.au_names_list) + len(PRESET_VARS.Hidden_AUs)):
            ...
            projected = self.transformation_layers[i_au](au_metric)
