...
class Tester_Single_task(object):
    def __init__(self, model, task, annotation_file, save_dir, video_dir, alpha, video=False, seq_len = None):
        self.model = model
        self.task = task
    ...
    def test_one_video(self, data_loader, task = 'AU'):
        ...
        track_metrics_alpha = temporal_smooth_function(track_metrics, self.alpha)
...
class Tester_MTL(Tester_Single_task):
    def __init__(self, model,  annotation_file, save_dir, alpha, video=False, seq_len = None):
        self.model = model
        self.annotation_file = annotation_file
        self.save_dir = save_dir
        self.video = video
    ...
    def test_one_video(self, data_loader):
        ...
        tasks = ['AU', 'EXPR', 'VA']
        track_metrics = dict([(t, []) for t in tasks])
        total_estimates = dict([(t, []) for t in tasks])
        with torch.no_grad():
            for i_val_batch, val_batch in tqdm(enumerate(data_loader), total = len(data_loader)):
                # evaluate model
                images, labels = val_batch
                outputs, metrics = self.model.forward(images.cuda())
                for t in tasks:
                    track_metrics[t].append(metrics[t].detach().cpu())
        for task in track_metrics:
            track_metrics[task] = torch.cat(track_metrics[task], axis=0)
            i_classifier = self.model.tasks.index(task)
            track_metrics_alpha = temporal_smooth_function(track_metrics[task], self.alpha[task])
            preds_after_smooth = self.model.emotion_classifiers[i_classifier](track_metrics_alpha.cuda())
            o_task = preds_after_smooth.cpu()
            if task == 'AU':
                o = (torch.sigmoid(o_task.detach().cpu())>0.5).type(torch.LongTensor)
                ...
                estimates = o_task.detach().cpu().numpy()
            total_estimates[task] = estimates
...
if __name__ == '__main__':
    ...
    'EXPR', 
    'VA']
    alphas = {'AU': 7, 'EXPR':9, 'VA':9}
    for task, annotation_file in zip(tasks, annotation_files):
        Tester_Single_task(model, task, annotation_file, args.save_dir, args.video_dir, alphas[task])

    annotation_file = 'create_annotation_file/MTL/MTL_test_set.pkl'
    Tester_MTL(model, annotation_file, args.save_dir, alphas)
