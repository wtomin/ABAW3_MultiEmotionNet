...
class Multitask_EmotionNet(InceptionV3MTModel):
    ...
    def configure_architecture(self):
        ...
        self.transformation_matrices_EXPR = []
        ...
        for i_au in range(len(self.au_names_list) + len(PRESET_VARS.Hidden_AUs)):
            ...
            self.transformation_matrices_EXPR.append(matrix)
        self.transformation_matrices_EXPR = nn.ModuleList(self.transformation_matrices_EXPR)

        self.transformation_matrices_VA = []

        for i_au in range(len(self.au_names_list) + len(PRESET_VARS.Hidden_AUs)):
            matrix = nn.Linear(self.AU_metric_dim, self.AU_metric_dim, bias=False)
            self.transformation_matrices_VA.append(matrix)
        self.transformation_matrices_VA = nn.ModuleList(self.transformation_matrices_VA)
    ...
    def forward(self, x):
        ...
        for i_au in range(len(self.au_names_list) + len(PRESET_VARS.Hidden_AUs)):
            ...
            projected = self.transformation_matrices_EXPR[i_au](au_metric)
            EXPR_metrics.append(projected)
        EXPR_metrics = torch.stack(EXPR_metrics, dim=1) # bs, numeber of regions, dim
        
        VA_metrics = []
        for i_au in range(len(self.au_names_list) + len(PRESET_VARS.Hidden_AUs)):
            au_metric = AU_metrics[:, i_au, ...]
            projected = self.transformation_matrices_VA[i_au](au_metric)
            VA_metrics.append(projected)
        VA_metrics = torch.stack(VA_metrics, dim=1) # bs, numeber of regions, dim

        if not self.avg_features:
            bs, length = EXPR_metrics.size(0), EXPR_metrics.size(1)
            # EXPR classifier
            i_classifier = self.tasks.index('EXPR')
            outputs['EXPR'] = self.emotion_classifiers[i_classifier](EXPR_metrics.view((bs*length, -1))).view((bs, length, -1))
            metrics['EXPR'] = EXPR_metrics

            # VA classifier
            i_classifier = self.tasks.index('VA')
            outputs['VA'] = self.emotion_classifiers[i_classifier](VA_metrics.view((bs*length, -1))).view((bs, length, -1))
            metrics['VA'] = VA_metrics
        else:
            EXPR_metrics = EXPR_metrics.mean(1)
            # EXPR classifier
            i_classifier = self.tasks.index('EXPR')
            outputs['EXPR'] = self.emotion_classifiers[i_classifier](EXPR_metrics)
            metrics['EXPR'] = EXPR_metrics
            
            VA_metrics = VA_metrics.mean(1)
            # VA classifier
            i_classifier = self.tasks.index('VA')
