...
class Multitask_EmotionNet(InceptionV3MTModel):
    ...
    def configure_architecture(self):
        ...
        self.transformation_matrices =nn.Sequential(
                nn.Linear(self.AU_metric_dim*N_regions, self.AU_metric_dim, bias=False),
                nn.ReLU())
    ...
    def forward(self, x):
        ...
        bs, N_regions = output_seq.size(0), output_seq.size(1)
        EXPR_VA_metrics = self.transformation_matrices(output_seq.view((bs, -1))) # concatenation 
        #assert self.avg_features, "avg_features has to be True: concatenation of features then learn a linear matrix with one ReLU"
